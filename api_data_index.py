...
# API URLs for pedestrian count data
...
def fetch_api_data():
    """Fetch pedestrian count data from the ArcGIS API."""
    try:
        response = requests.get(PEDESTRIAN_API_URL)
        response.raise_for_status()
        return response.json()
    except requests.RequestException as e:
        logger.error(f"Error fetching pedestrian data from API: {e}")
        return None

def transform_api_data(data):
    """Transform pedestrian count API data into Elasticsearch bulk format."""
    actions = []
...
def index_api_data():
    """Fetch, transform, and index pedestrian count data into Elasticsearch."""
    logger.info("Fetching pedestrian count data...")
    data = fetch_api_data()
    if not data:
        logger.error("No pedestrian data fetched from API.")
        return

    logger.info("Transforming pedestrian count data...")
    actions = transform_api_data(data)
    try:
        helpers.bulk(es, actions)
        logger.info("Pedestrian count data indexed successfully")
...
def schedule_api_updates():
    """Schedule periodic updates for pedestrian count data indexing."""
    scheduler = BlockingScheduler()
    # Schedule the job to run every hour
    scheduler.add_job(index_api_data, 'interval', hours=1)
    logger.info("Scheduler started. Pedestrian count data will be updated every hour.")
    try:
        scheduler.start()
...
if __name__ == "__main__":
    ...
    logger.info("Indexing pedestrian count data immediately...")
    index_api_data()

    # Start the scheduler to keep the pedestrian count data up to date
